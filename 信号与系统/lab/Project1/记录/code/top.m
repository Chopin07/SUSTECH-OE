...
% array_start_time = [0:0.1:9.9];                             %每隔0.5s截取一段数据
...
% 
%     % 提取所需的时间段数据
%     seq_ref_window = seq_ref(1, 1 + offset:offset + duration / N * f_s);
%     seq_sur_window = seq_sur(1, 1 + offset:offset + duration / N * f_s);    
%     % idx_start_time = 1;
%     % idx_start_time
% 
%     %% Digital downconvert                            %数字下变频
%     fprintf('[stat]   Downconvert.\n')
%     seq_ref_ddc = seq_ref_window.*exp(-1i*2*pi*f_ddc*[0:duration / N *f_s-1]/f_s);
%     seq_sur_ddc = seq_sur_window.*exp(-1i*2*pi*f_ddc*[0:duration / N *f_s-1]/f_s);
...
for idx_start_time = 1:length(array_start_time) 
    ...
    %% Digital downconvert                            %数字下变频
    fprintf('[stat]   Downconvert.\n')
    seq_ref_ddc = seq_ref.*exp(-1i*2*pi*f_ddc*[0:duration / N *f_s-1]/f_s);
    seq_sur_ddc = seq_sur.*exp(-1i*2*pi*f_ddc*[0:duration / N *f_s-1]/f_s);
   ...
   tic
    ...
    t_axis = 0:1/f_s:(duration / N*f_s-1)/f_s;
...
[meshgrid_Doppler,meshgrid_start_time] = meshgrid(array_Doppler_frequency,[array_start_time,array_start_time(end)+duration / N]);
...
xticks([array_Doppler_frequency(1):20:array_Doppler_frequency(end)])
yticks([array_start_time(1):0.5:array_start_time(end)+duration / N])
xlabel('Doppler frequency (Hz)')
ylabel('Time (s)')
...
%     meshgrid(array_Doppler_frequency,[array_start_time,array_start_time(end)+duration / N]);
...
% fig104 = figure(104);
% ScreenSize = get(0,'ScreenSize');
% set(fig104,'Position',[0.5*ScreenSize(3)+50,50,0.25*ScreenSize(3)-100,0.5*ScreenSize(4)-150]);
% [meshgrid_Doppler,meshgrid_start_time] = ...
%     meshgrid(array_Doppler_frequency,[array_start_time,array_start_time(end)+duration / N]);
% plot_A_TD = zeros(length(array_start_time),length(array_Doppler_frequency));
% 
