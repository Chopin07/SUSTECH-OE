...
for idx_start_time = 1:length(array_start_time)
    ...
    % 提取所需的时间段数据
    seq_ref_window = seq_ref(1, 1 + offset:offset + duration/5 * f_s);
    seq_sur_window = seq_sur(1, 1 + offset:offset + duration/5 * f_s);    
    % idx_start_time = 1;
    % idx_start_time

    %% Digital downconvert                            %数字下变频
    fprintf('[stat]   Downconvert.\n')
    seq_ref_ddc = seq_ref_window.*exp(-1i*2*pi*f_ddc*[0:duration/5*f_s-1]/f_s);
    seq_sur_ddc = seq_sur_window.*exp(-1i*2*pi*f_ddc*[0:duration/5*f_s-1]/f_s);
   ...
   % Ambiguity function                             
    ...
    t_axis = 0:1/f_s:(duration/5*f_s-1)/f_s;
...
[meshgrid_Doppler,meshgrid_start_time] = ...
    meshgrid(array_Doppler_frequency,[array_start_time,array_start_time(end)+duration/5]);
...
xticks([array_Doppler_frequency(1):20:array_Doppler_frequency(end)])
yticks([array_start_time(1):0.5:array_start_time(end)+duration/5])
xlabel('Doppler frequency (Hz)')
ylabel('Time (s)')
